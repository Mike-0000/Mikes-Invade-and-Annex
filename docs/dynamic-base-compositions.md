...
*Existing settings and the existing 1.75x garrison scaling still apply. Gun crews are deducted from that total, not added to it. Every recipe authors at least one socketed weapon on each sandbag face. About seven in ten constructions then leave one whole face empty so an approach can be scouted. One heavy position (NSV, scoped NSV or AA) is allowed on Full/Compact only; AA shares that allowance. Live installation can still omit a socket after access/firing validation.
