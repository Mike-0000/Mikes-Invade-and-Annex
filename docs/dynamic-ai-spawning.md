...
Open **Admin configuration → Dynamic AI**. The dedicated tab holds the master toggle, Dynamic AI scale, soldier budget, distances, and timing. The existing Scaling tab retains the saved AI multiplier, static override, and combat settings.
...
| --- | --- | --- | --- |
| Dynamic AI scale | 0.8 | 0.1–10 | Roster strength while Dynamic AI Spawning is ON. Independent of the Scaling-tab multiplier and static override. |
...
| Combat quiet time | 60 s | 10–300 s | Both modes: recent target/danger protection, only while a player is inside wake distance. |
...
2. A sliced census measures the minimum horizontal distance from any current live or saved soldier to **any player pawn**. Live members use each pawn's current world transform; unrestored reserves use their last saved pose. The native group-entity origin stays at spawn and is not the group location. Objective centers and the first connected player do not determine priority. Split player teams therefore create separate nearby fronts within the same shared budget.
...
5. Ordinary optional demand begins inside the wake distance, **1000 m by default**. An existing allocation may remain eligible out to the cache distance, **1500 m by default**, reducing boundary churn. If a closer wake-range squad still has reserves or unmet allocation, that outer incumbent eligibility is dropped so leftover slots are not kept on the farther group. These are eligibility distances, not unconditional full-squad wake distances in budget mode.
...
- Recent combat prevents eviction of a fighting squad for the combat quiet time, **60 seconds by default**, only while a player is inside wake distance **and** the shared target still has room **and** a closer squad is not waiting for those slots. Native selected targets, danger, and casualties do not count when every player is beyond wake. Once the target is full, a closer squad needs the slots, or if the admin hard cap is on, distance decides who stays. Combat never force-restores remaining cached teammates over the shared target. Capture waits for at least one physical defender in budget mode rather than the entire virtual roster. Defend orders apply to hybrid live members. Temporary spawn/order initialization protects current members without independently waking all reserves.
- Newly observed or recreated soldiers remain live for the configured minimum live time, **30 seconds by default**. A lower allocation must then remain pending for the reduction delay, **10 seconds by default**, before removal work can begin. At default settings, fresh groups commonly need roughly 40 seconds plus census/queue time before their first budget reduction. These controls are separate from the distance-only departure quiet time. **Nearby preemption:** when a closer wake-range squad still needs soldiers, a clearly farther overallocated group (farther by more than the retention preference) skips that 10-second delay, uses a **5-second** minimum live time for soldiers beyond the keep-release distance, and combat no longer holds it. Soldiers inside the release distance, injuries, roles, vehicles, occupancy, and admitted retries stay protected. Approach still does not restore a cached town over the budget.
...
- A supported team caches only after every member stays beyond the configured cache distance (**1500 m by default**) horizontally from every player for the departure quiet time (**60 continuous seconds by default**), with all whole-team safety checks satisfied. Distant native combat no longer blocks that cache.
...
2. **Approach after cache:** leave until infantry have cached at least once, then walk into the AO. Physical infantry should climb toward the budget and stop there. Nearest groups restore first; farther groups stay cached or get evicted **without waiting the ordinary 10-second reduction delay** when the closer squad still needs slots. A firefight should stay populated around that budget, not dump the whole town. Split players share the same cap across fronts. After withdrawal beyond wake, native combat or AI-vs-AI fire should not keep far infantry at the budget; expired min-live then frees those slots. While the live count is at the target, automatic capture QRF and AO reinforcement waves must not spawn; after casualties drop the count under the target, the next wave/QRF check may fire. Admin Force QRF and defend-mission waves must still spawn at a full budget.
